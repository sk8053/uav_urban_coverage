...
class ArrayBase(object):
    ...
    def sv(self, phi, theta,dly=0, include_elem=True, return_elem_gain=False, drone = False, print_phi= False):
        """
        ...
        if include_elem:
            if drone is True:
                ...
                phi, theta = np.array(phi, dtype = int), np.array(theta, dtype = int) # elevation angle to inclination angle
...
class RotatedArray(ArrayBase):
    ...
    def __init__(self, arr, phi0=0, theta0=0, drone = False):
        """
        Constructor
        Parameters
        ----------
